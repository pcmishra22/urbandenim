        ...
        <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('seo_title') is-invalid @enderror" 
                       id="seo_title" name="seo_title" value="{{ old('seo_title', $brand->seo_title) }}"
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('seo_description') is-invalid @enderror" 
                          ...
                          placeholder="SEO description...">{{ old('seo_description', $brand->seo_description) }}</textarea>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('seo_keywords') is-invalid @enderror" 
                          ...
                          placeholder="jeans, denim, ...">{{ old('seo_keywords', $brand->seo_keywords) }}</textarea>
                @error('seo_keywords')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="is_active" class="form-select @error('is_active') is-invalid @enderror">
                        <option value="1" @selected((string) old('is_active', $brand->is_active) === '1')>Active</option>
                        <option value="0" @selected((string) old('is_active', $brand->is_active) === '0')>Inactive</option>
                    </select>
                    @error('is_active')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="is_featured" class="form-select @error('is_featured') is-invalid @enderror">
                        <option value="1" @selected((string) old('is_featured', $brand->is_featured) === '1')>Yes</option>
                        <option value="0" @selected((string) old('is_featured', $brand->is_featured) === '0')>No</option>
                    </select>
                    @error('is_featured')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
